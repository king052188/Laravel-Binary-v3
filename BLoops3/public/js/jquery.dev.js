...
function populate_genealogy_history(account, IsRefresh) {
    $(document).ready(function() {
        $.ajax({
            dataType: 'json',
            type:'POST',
            url: '/genealogy/member-structure-details' + account,
            beforeSend: function () {
              if(!IsRefresh) {
                html = "<tr>";
                html += "<td>***</td>";
...
function populate_pairing_history(account) {
    $(document).ready(function() {
        $.ajax({
            dataType: 'json',
            type:'POST',
            url: '/genealogy/member-pairing-details' + account
        }).done(function(json){
            // console.log(json);
            var html = "";
            var profit = 0;
            ...
            html += "<tr>";
            html += "<td colspan='3' style='text-align: right; padding: 5px; font-weight: 600;'>Total</td>";
            html += "<td style='text-align: right;'>"+numeral(json.Total_Amount).format('0,0.00')+"</td>";
